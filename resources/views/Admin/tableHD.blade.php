                                ...
                                <div class="bootstrap-data-table-panel">
                                    <div class="table-responsive">
                                        <table id="row-select" class="display table table-borderd table-hover">
                                            ...
                                            <tbody>
                                                @foreach($DSBill as $items)
                                                <tr>
                                                    <td>{{$items->mahd}}</td>
                                                    <td>{{$items->username}}</td>
                                                    <td>{{$items->soluong}}</td>
                                                    <td>{{$items->thanhtoan}}</td>
                                                    <td>{{$items->tongtien}} VND</td>
                                                    <td>
                                                        @if($items->tinhtrang=="Chờ xác nhận")
                                                        <form action="/tableHD/{{$items->mahd}}" method="post">
                                                            @csrf
                                                            <button type="submit"
                                                                class="btn btn-danger btn-rounded m-b-10 m-l-5">Xác nhận đơn hàng</button>
                                                        </form>
                                                        @elseif($items->tinhtrang=="Đang giao")
                                                        <form action="/tableHD/{{$items->mahd}}" method="post">
                                                            @csrf
                                                            <button type="submit"
                                                                class="btn btn-info btn-rounded m-b-10 m-l-5">Đang giao</button>
                                                        </form>
                                                        @elseif($items->tinhtrang=="Đã giao")
                                                            <button
                                                                class="btn btn-success btn-rounded m-b-10 m-l-5">Đã giao</button>
                                                        @endif
                                                    </td>
                                                    <td>
                                                        <form action="/tableCTHD/{{$items->mahd}}" method="get">
                                                            @csrf
                                                            <button type="submit"
